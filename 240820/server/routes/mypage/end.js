...
mypagerou.post('/bchange', async (req, res) => {
  ...
  try {
    ...
    // res.json(solar_board);
    console.log('피카츄', solar_board.b_state[0]);
    console.log('피카츄', typeof solar_board.b_state[0]);

    
    if (solar_board.b_state[0] == 'ing'){
      const sql2 = `update TB_SOLAR_BOARD set B_STATE = 'end' where MEM_ID = '${id}'`;
      const result2 = await connection.execute(sql2);
      console.log("마이멜로디 결과", result2.rowsAffected);
        if (result2.rowsAffected && result2.rowsAffected > 0) {
            res.json('success');
    ...
    }else if (solar_board.b_state[0] == 'end'){
      const sql2 = `update TB_SOLAR_BOARD set B_STATE = 'ing' where MEM_ID = '${id}'`;
      const result2 = await connection.execute(sql2);
      console.log("쉐이미 update문 결과", result2.rowsAffected);
        if (result2.rowsAffected && result2.rowsAffected > 0) {
            res.json('success');
...
// 4. '/waitstate' : 마이페이지 승인 완료 / 승인 대기 상태 파악
mypagerou.post('/waitstate', async (req, res) => {
  const { userInfo } = req.body;
...
mypagerou.post('/mypagewait', async (req, res) => {
  ...
  try {
    ...
    console.log('뮤츠', solarplant.b_state[idx-1]);
    if (solarplant.b_state[idx-1] == 'wait'){
      ...
      if (result2.rowsAffected > 0) {
        ...
        console.log('승인 버튼 변경 실패');
        res.json('failed');
      }

    }else if (solarplant.b_state[idx-1] == 'complete'){
      const sql2 = `update TB_SOLARPLANT set B_STATE = 'wait' where MEM_ID = '${solarplant.mem_id[idx-1]}'`
      if (!sql2) {
        return res.status(400).json({ error: 'SQL query is required' });
      }

      console.log('이브이', sql2);
      const result2 = await connection.execute(sql2);
      console.log('님피아', result2.rowsAffected);
